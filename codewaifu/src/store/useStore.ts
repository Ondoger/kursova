...
import type { CharacterMood, GitHubStats } from '../types';
...
interface AppState {
  ...
  connectWithGitHub: (token: string) => Promise<void>;
  refresh: () => Promise<void>;
  logout: () => void;
  setMood: (mood: CharacterMood) => void;
  triggerMood: (mood: CharacterMood, durationMs?: number) => void;
  setCharacterName: (name: string) => void;
  setUnlocked: (ids: string[]) => void;
  setLastSeenLevel: (lvl: number) => void;
}
  ...
  persist(
    (set, get) => ({
      ...
      setCharacterName: (name) =>
        set({ characterName: name.trim() || 'CodeWaifu' }),
