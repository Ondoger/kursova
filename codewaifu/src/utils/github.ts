...
const cacheKey = (username: string) => `codewaifu:stats:${username.toLowerCase()}`;
...
async function gh<T>(path: string, token?: string): Promise<T> {
  ...
  if (!res.ok) {
    if (res.status === 401) throw new Error('Недійсний GitHub token або сесія GitHub завершилась.');
    if (res.status === 404) throw new Error('Користувача не знайдено');
    if (res.status === 403) throw new Error('Перевищено ліміт GitHub API. Спробуй з токеном.');
...
async function fetchUser(username: string, token?: string): Promise<GitHubUser> {
  return gh<GitHubUser>(`/users/${encodeURIComponent(username)}`, token);
}
...
async function fetchRepos(username: string, token?: string): Promise<GitHubRepo[]> {
  ...
  for (let page = 1; page <= 4; page++) {
    const part = await gh<GitHubRepo[]>(
...
async function fetchEvents(username: string, token?: string): Promise<GitHubEvent[]> {
  ...
  for (let page = 1; page <= 3; page++) {
    try {
      const part = await gh<GitHubEvent[]>(
...
function computeStreaks(commitsByDay: Record<string, number>) {
...
export async function fetchGitHubStats(
  ...
  if (!options.force) {
    try {
      ...
      if (raw) {
        const cached: GitHubStats = JSON.parse(raw);
...
export async function fetchLeaderboard(token?: string): Promise<LeaderboardEntry[]> {
  ...
  try {
    ...
    if (raw) {
      const cached = JSON.parse(raw) as { at: number; data: LeaderboardEntry[] };
  ...
  const data = await gh<{ items: Array<{ login: string; avatar_url: string; html_url: string }> }>(
  ...
  const enriched: LeaderboardEntry[] = await Promise.all(
    data.items.map(async (u) => {
      try {
        const full = await gh<GitHubUser>(`/users/${u.login}`, token);
        return {
          login: full.login,
