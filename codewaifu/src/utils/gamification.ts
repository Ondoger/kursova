import type { CharacterTier, GitHubStats } from '../types';

export const calculateXP = (stats: GitHubStats): number => {
  return (
...
export const getTierForLevel = (level: number): CharacterTier => {
  return (
    CHARACTER_TIERS.find((t) => level >= t.min && level <= t.max) ??
    CHARACTER_TIERS[0]
  );
};

export const computeRPGStats = (stats: GitHubStats) => {
  const recentCommits = Object.values(stats.commitsByDay).reduce(
    (a, b) => a + b,
    0,
  );
  ...
  const agi = Math.min(
    100,
    Math.round((stats.prsMerged * 8 + stats.prsOpened * 4) || stats.totalRepos * 4),
  );
