...
import type { CharacterMood, CharacterTier } from '../types';
 ...
 */
export function CodeWaifu({ tier, mood, level }: CharacterProps) {
  ...
  const hairTwinL = useRef<THREE.Group>(null);
  const hairTwinR = useRef<THREE.Group>(null);
  ...
  useFrame((state) => {
    ...
    if (torso.current) torso.current.scale.y = 1 + breath * 0.5;
    if (head.current) {
    ...
    // Hair floating
    if (hairBack.current) hairBack.current.rotation.x = Math.sin(t * 1.2) * 0.05;
    ...
    // Default arm rest
    if (armL.current) armL.current.rotation.z = 0.18 + Math.sin(t * 1.2) * 0.04;
    if (armR.current) armR.current.rotation.z = -0.18 - Math.sin(t * 1.2 + 0.6) * 0.04;
    ...
    if (mood === 'victory') {
      ...
      if (armL.current) armL.current.rotation.z = 1.4 + k * 0.2;
      if (armR.current) armR.current.rotation.z = -1.4 - k * 0.2;
      if (root.current) root.current.position.y += Math.abs(Math.sin(t * 8)) * 0.2;
      ...
      if (armL.current) armL.current.rotation.z = 1.7;
      if (armR.current) armR.current.rotation.z = -1.7;
      void spin;
    } else if (mood === 'sad') {
      if (head.current) {
      ...
      if (armL.current) armL.current.rotation.z = 0.05;
      if (armR.current) armR.current.rotation.z = -0.05;
      if (root.current) root.current.position.y -= 0.05;
  ...
  return (
    <group ref={root} position={[0, -0.2, 0]}>
      ...
      {/* Aura sphere */}
      {tier.hasAura && (
        <mesh ref={aura} position={[0, 0.2, 0]}>
      ...
      {/* Wings */}
      {tier.hasWings && (
        <group ref={wings} position={[0, 0.5, -0.25]}>
      ...
      <group ref={head} position={[0, 1.55, 0]}>
        ...
        <mesh position={[0, 0.32, 0.18]}>
          ...
          <meshStandardMaterial
            color={hairColor}
            emissive={hairColor}
            emissiveIntensity={tier.hasAura ? 0.4 : 0.15}
            roughness={0.4}
          />
        </mesh>
        {/* Side bangs */}
        ...
        <group ref={hairBack} position={[0, 0.05, -0.05]}>
          ...
          <mesh position={[0, -0.45, -0.18]}>
            ...
            <meshStandardMaterial
              color={hairColor}
              emissive={hairColor}
              emissiveIntensity={tier.hasAura ? 0.35 : 0.1}
              roughness={0.5}
            />
          </mesh>
        </group>
        ...
        <group ref={hairTwinL} position={[-0.42, 0.1, -0.05]}>
          <mesh position={[0, -0.4, 0]} rotation={[0, 0, 0.35]}>
            ...
            <meshStandardMaterial
              color={hairColor}
              emissive={hairColor}
              emissiveIntensity={tier.hasAura ? 0.4 : 0.12}
              roughness={0.45}
            />
          </mesh>
...
function Wing({ side, color, accent }: { side: 'left' | 'right'; color: string; accent: string }) {
  const sign = side === 'left' ? -1 : 1;
  return (
    <group position={[sign * 0.55, 0, -0.35]} rotation={[0, sign * 0.5, 0]}>
