          ...
          <form onSubmit={handleSubmit} className="space-y-3 max-w-md">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-4 rounded-xl bg-white/[0.04] border border-white/10 focus:border-neon-pink/60 focus:bg-white/[0.06] focus:outline-none focus:ring-4 focus:ring-neon-pink/10 transition-all font-medium placeholder:text-white/30"
              />
            </div>
            ...
            <NeonButton
              ...
              disabled={loading || oauthLoading || !oauthConfigured}
              variant="primary"
            ...
            {!oauthConfigured && (
              <div className="text-xs text-white/45 bg-white/[0.04] border border-white/10 px-3 py-2 rounded-lg leading-relaxed">
            ...
            {deviceCode && (
              <motion.div
                ...
                className="text-sm text-white/70 bg-neon-cyan/5 border border-neon-cyan/30 px-3 py-3 rounded-lg"
              >
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-4 rounded-xl bg-white/[0.04] border border-white/10 focus:border-neon-cyan/60 focus:bg-white/[0.06] focus:outline-none focus:ring-4 focus:ring-neon-cyan/10 transition-all font-medium placeholder:text-white/30"
                autoFocus
              />
              <span className="absolute inset-0 rounded-xl pointer-events-none opacity-0 group-focus-within:opacity-100 transition-opacity"
                style={{ boxShadow: '0 0 30px rgba(0,255,255,0.25)' }}
              />
            </div>
            ...
            {showAdvanced && (
              <input
                ...
                placeholder="ghp_..."
                type="password"
                className="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 focus:border-neon-purple/60 focus:outline-none transition-all font-mono text-sm placeholder:text-white/20"
              />
            )}
            ...
            {(error || authError) && (
              <motion.div
                initial={{ opacity: 0, y: -8 }}
                animate={{ opacity: 1, y: 0 }}
                className="text-sm text-neon-pink bg-neon-pink/5 border border-neon-pink/30 px-3 py-2 rounded-lg"
              >
                {error || authError}
              </motion.div>
            )}
