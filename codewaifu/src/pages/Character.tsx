      ...
      <div className="grid lg:grid-cols-[1fr_420px] gap-6">
        ...
        <div className="space-y-4">
          <GlassCard glow="pink" hoverable={false}>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Назви свого персонажа"
                className="min-w-0 flex-1 px-4 py-3 rounded-xl bg-white/[0.04] border border-white/10 focus:border-neon-pink/60 focus:outline-none transition-all"
              />
              <NeonButton
                variant="pink"
                onClick={() => setCharacterName(nameDraft)}
                disabled={nameDraft.trim() === characterName}
          ...
          <GlassCard glow="gold" hoverable={false}>
            ...
            {skills.length === 0 ? (
              <p className="text-sm text-white/40">Немає даних</p>
            ) : (
          ...
          <GlassCard glow="cyan" hoverable={false}>
            ...
            <ul className="space-y-2">
              {CHARACTER_TIERS.map((tier) => {
                const isCurrent =
                  data.level >= tier.min && data.level <= tier.max;
                return (
                  <li
                    key={tier.name}
