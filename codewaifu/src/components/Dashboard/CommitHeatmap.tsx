...
export function CommitHeatmap() {
  ...
  const colorFor = (count: number) => {
    ...
    if (ratio < 0.25) return `rgba(124, 58, 237, ${intensity})`;
    if (ratio < 0.5) return `rgba(168, 85, 247, ${intensity})`;
    if (ratio < 0.75) return `rgba(0, 255, 255, ${intensity})`;
    return `rgba(255, 0, 255, ${intensity})`;
  };

  const shadowFor = (count: number) => {
    if (count <= 0) return 'none';
    const ratio = max ? Math.min(1, count / max) : 0;
    if (ratio < 0.5) return `0 0 4px rgba(124,58,237,0.5)`;
