...
export function Navbar() {
  ...
  return (
    <motion.header
      ...
      className="sticky top-0 z-40 backdrop-blur-xl bg-bg-900/60 border-b border-white/5"
    >
                ...
                <>
                  ...
                  {isActive && (
                    <motion.span
                      ...
                      className="absolute inset-0 rounded-lg bg-white/[0.06] border border-neon-cyan/40"
                      style={{ boxShadow: '0 0 16px rgba(0,255,255,0.25)' }}
                    />
                  )}
                </>
              )}
        ...
        {username && (
          <div className="flex items-center gap-3">
            <button
              onClick={() => refresh()}
              disabled={loading}
              className="text-white/60 hover:text-neon-cyan disabled:opacity-50 transition-colors text-sm"
              title="Оновити"
            >
              {loading ? '' : ''}
            </button>
            ...
            <button
              onClick={logout}
              className="text-xs text-white/40 hover:text-neon-pink transition-colors"
            >
              Вийти
            </button>
          </div>
